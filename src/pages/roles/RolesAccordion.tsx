...
const ROWS: RoleRow[] = [
  ...
  },
  {
    ...
    body: [
      "List certificates as collateral on lending markets, build uTSLA/USDG pairs, structure products on top of a spot asset that never needed to exist off chain. Certificates are plain tokens: permissionless to integrate, oracle-priced from the same feed the vaults use.",
  ...
  },
  {
    ...
    body: [
      HAS_CERT_TOKEN
        ? "Live, in its first form: the insurance pool (InsuranceStaking) takes USDG, not CERT. It is unaudited and capped at 10,000 USDG, and it has no automatic income yet. CERT can be staked separately (CertStaking) for a share of the buyback fund's fee income; it is never drawn, and it pays only when funded."
