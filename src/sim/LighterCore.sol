...
abstract contract LighterCore is ILighter {
    ...
    ///
    ///      BEHAVIOUR IS UNCHANGED, deliberately and in every particular:
    ///
    ...
    ///        * It reads CASH (`marginBalanceOf`), not equity. A real venue's initial margin is met
    ///          with posted collateral, and every fill here happens at the mark it is valued
    ///          against, so an increase carries no PnL of its own to credit.
    ///        * Task 7's fix is preserved exactly: the cash is `marginBalanceOf[account]`, the
    ///          SUBMITTING account's own, never a pool. That one read is what stopped an account
    ///          holding 1 USDG from opening a $10,000 position on everyone else's margin.
        ...
        returns (bool)
    {
        ...
        uint256 cash = marginBalanceOf[account];
