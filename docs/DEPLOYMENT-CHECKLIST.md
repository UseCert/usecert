...
| --- | --- | --- |
| `CertOracle.feed` | A real Chainlink-compatible aggregator, and specifically **the Chainlink proxy address, never the underlying aggregator**. Immutable | Wired to an underlying aggregator, the deployment inherits that aggregator's own round numbering, which restarts at 1 on every rotation — see the next row, which is what makes this a safety item rather than a convenience one |
| `feed`'s `roundId` | **Must be strictly non-decreasing for the life of the deployment.** Since Task 1 this is *security* load-bearing, not cosmetic: `pokeLastGood`'s round-distinctness proof is `roundId > pendingRoundId`, so H-1's reference can only advance when the feed's round number goes up. A Chainlink **proxy** satisfies this by construction — it packs `phaseId` into the high 16 bits of the `uint80`, so a Chainlink-initiated aggregator rotation *increases* `roundId` across the rotation. An underlying aggregator read directly, a feed whose numbering resets, or a simulator serving a constant `roundId` does not | `pokeLastGood` reverts `CertOracle_ReferenceRoundNotAdvanced` for as long as the feed serves `roundId <= pendingRoundId`, so **a large sustained repricing can never be absorbed and minting stays paused until a redeploy.** Measured: a constant `roundId` freezes the reference permanently; a reset while a candidate is armed freezes it until price re-enters `deviationBps` of the reference, at which point the in-band path refreshes `lastGoodPx18` and clears the stale `pendingRoundId`. **Fails closed** — the breaker only ever gets stricter, never looser, and redemption is untouched throughout (Law 2 reads `pxUnguarded()`). The symptom is a permanent mint pause with redemption working, which is easy to misdiagnose |
| `feed`'s `answeredInRound` | **Considered, and deliberately still discarded.** `latestRoundData()`'s 5th return value is what would flag a carried-over answer (`answeredInRound < roundId`); `_readFeed` and `_tryFeed` read neither. Not needed, because the two gates in front of every reference advance already refuse that observation: `stalenessSeconds` rejects it when the feed stops re-reporting, and the strict `roundId > pendingRoundId` advance rejects a republished answer carrying no new round. Recorded rather than left silent, because Task 1 made the *other* member of the same tuple load-bearing | None today. It becomes a real gap only for a feed whose `roundId` advances while `answeredInRound` lags, which is a `_tryFeed` change, not a configuration choice |
| `feed.decimals()` | **Must be ≤ 36, and must not change.** **RESOLVED (Task 3) — the `_tryFeed`/`_readFeed` asymmetry this row used to record is closed.** Both functions now bound `d` at 36, from the same literal: `_tryFeed` treats anything above as an unusable feed (so `pxUnguarded()`, `basisBps()` and `mintAllowed()` fall back rather than revert), and `_readFeed` — which `px()`, and therefore both mint paths, and the constructor use — reverts `CertOracle_FeedDecimalsOutOfRange`. Previously `_readFeed` had no bound, so `decimals() >= 96` overflowed `10 ** (d - 18)` and killed minting with an anonymous panic (0x11). A feed already out of range at deploy time is now refused **by name in the constructor** instead of panicking there, so this row is enforced at deployment rather than only documented | Minting is paused with a named error a caller can switch on. Redemption is unaffected and never was: `forceExit` prices off `pxUnguarded()`, which never reaches `_readFeed`. Measured — with the new bound reverted, `test_pxUnguardedSurvivesAbsurdFeedDecimals`, `test_forceExitSurvivesFutureFeedTimestamp` and `test_redeemSucceedsWithBufferAtZeroAndOracleStale` all still pass, and `test_forceExitSurvivesAbsurdFeedDecimals` proves a holder exits at 96 decimals with real margin posted and a real position open |
| `CertOracle.singleSource` | **The sharpest pre-deploy gate on this list, and no contract can check it.** `true` declares that `feed` and the venue mark are **not independent** — the market has no Chainlink feed and the venue's own mark, directly or through a venue-sourced adapter, is the only price there is. **28 of the venue's 57 perp markets are in that state — 20.5% of open interest**, including XAU (gold, $12.5M OI, the largest single gap), XAG, ANTHROPIC ($4.88M OI on $19.3M daily volume), OPENAI and SHEIN, all of which are in scope. **Set it `true` for every market without a Chainlink feed, and verify `oracle.singleSource()` against the feed you actually wired before funding anything.** Nothing on-chain can tell whether the configured `feed` is genuinely independent of the venue, so this is a human assertion and the whole guard rests on it. In `true` mode the guard set is deliberately smaller (see the next row); in `false` mode behaviour is bit-identical to before Task 2 | Set to `false` on a venue-derived feed and the deployment fails **silently**, which is what Task 2 exists to close: `basisBpsChecked()` returns `known = true, bps = 0` — a healthy basis *asserted, never computed* — and two of `mintAllowed()`'s three guards degenerate because the feed and the cross-check are the same number, leaving only the deviation clamp, which is a rate limit and not a truth check. Pushing the venue mark moves a venue-sourced feed and `markPx18` **together** and holds the basis at zero, so the degenerate band reads healthiest exactly while it is being defeated (ANTHROPIC's capacity is ~$486k against $4.88M of OI, so the mark is movable in practice). No contract, test or checklist item flags it. Set to `true` on a genuinely independent feed and you merely lose the basis band — a real guard, discarded, but it fails closed |
| `deviationBps` **when `singleSource == true`** | **Must be ≤ `CertOracle.MAX_SINGLE_SOURCE_DEVIATION_BPS` = 200 bps (2%)** — enforced at construction (`CertOracle_DeviationTooWideForSingleSource`), and **not configurable**: a deployment that wants a wider clamp must find a second price source instead. In single-source mode the clamp is the only remaining defence and its width is the entire per-window budget an attacker gets. 200 bps is roughly 4x the widest honest basis measured live (11.3–49.3 bps across 13 markets), so it does not fire on ordinary venue noise. `CertOracle.lastGoodPx18` must also be non-zero, or `mintAllowed()` fails closed — reachable only if construction itself normalised to zero, and repaired permissionlessly by `pokeLastGood()`'s bootstrap path | Refused at deployment, so unreachable. The residual risk is not the bound but the mode: see the row above |
| `basisBandBps` **when `singleSource == true`** | **Ignored, deliberately.** `mintAllowed()` skips the basis band *and its `markPx18 != 0` precondition* entirely in this mode, and `basisBps()` reverts `CertOracle_NoIndependentBasis` while `basisBpsChecked()` reports `known = false` — absent, not vacuous. Set the value to whatever documents intent; nothing reads it. **Any dashboard or monitor wired to `basisBps()` must handle the revert, and any monitor must key off `known` rather than off the bps value.** The same dropped precondition means `mintAllowed() == true` while `markPx18 == 0` is reachable for the first time in this mode — any monitor carrying the implicit invariant "minting open implies a mark is attested" is wrong here | A monitor that reads the bps number alone sees `0` and renders a perfectly tracking mark. That is the L-5 defect in the one form L-5's own fix could not see, and it is why `known = false` rather than `known = true, bps = 0` is the load-bearing part of Task 2 |
| Feed answer magnitude | **RESOLVED (Task 13) — the `_tryFeed`/`_readFeed` asymmetry this row used to record is closed, the same way Task 3 closed it for `decimals()`.** Normalisation is now guarded on **both** functions, so an unrepresentable answer makes the feed unusable rather than panicking: `_tryFeed` has guarded the product `uint256(answer) * (10 ** (18 - d))` since the CRITICAL B re-audit, and `_readFeed` — which `px()`, and therefore both mint paths, and the constructor use — now reverts `CertOracle_AnswerNotNormalisable` for the same condition instead of panicking (0x11). Previously `_readFeed` had no guard on the product, only on the exponent (Task 3's `d > 36`), so any `d <= 17` feed with an answer above `type(uint256).max / 10**(18-d)` — ~1.16e59 at `d = 0`, the worst case — panicked anonymously. Unreachable for `d` in 18..36, where the multiplier is 1 or the branch divides. A feed already unnormalisable at deploy time is now refused **by name in the constructor** instead of panicking there, exactly as Task 3 did for the decimals case. **Finding 1** made the vault's own valuation total for every price the feed can return, so an absurd print no longer reverts `forceExit` | Minting is paused with a named error a caller can switch on. Redemption is unaffected and never was: `forceExit` prices off `pxUnguarded()`, which never reaches `_readFeed` and has been guarded against this exact product since the CRITICAL B re-audit. Measured — `test_pxRevertsNamedOnAnswerNotNormalisable` proves the named revert on `px()` and that `pxUnguarded()` still falls back to the last-good snapshot in the same state, `test_constructorRevertsNamedOnAnswerNotNormalisable` proves the constructor is refused by name, and `test_readFeedAnswerNotNormalisableUnreachableAtD18` proves the exact boundary at `d == 18` returns cleanly rather than reverting |
